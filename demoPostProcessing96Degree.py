...
def demo():
    ...
    '''input'''
    lmax = 96  # max degree/order
    grid_space = 1  # Equal spacing (degree) of L3 gridded output

    begin_date, end_date = datetime.date(2002, 1, 1), datetime.date(2025, 12, 31)
    ...
    gsm_path = FileTool.get_project_dir(sub="data/L2_SH_products/GSM/CSR/RL06/BB01/CSR_RL06_GSM_BB01")
    ...
    filter_method, filter_params = Enums.SHCFilterType.DDK, (2,)
    ...
    basin_path_shp = FileTool.get_project_dir("data/basin_mask/Shp/AOI_shp")
    ...
    Simple scripts of pyplot can help you quickly check the results.
     '''
    # Temporal plot with title/labels/legend + YEARLY ticks + yearly grid 
    year_frac = TimeTool.convert_date_format(dates_ave, output_type=TimeTool.DateFormat.YearFraction)

    plt.figure(figsize=(10, 4))
    plt.plot(year_frac, ewh[0], label="EWH (m)")
    plt.title("NSW Basin Mean (EWH)- DDK-2")
    plt.xlabel("Year")
    plt.ylabel("EWH (m)")
    plt.legend()

    ax = plt.gca()
    ax.xaxis.set_major_locator(mticker.MultipleLocator(1.0))   # tick every 1 year
